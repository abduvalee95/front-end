...
/**
 * The product's only surface primitive.
 *
 * One radius (`--radius-card`), one border colour, one elevation
 * (`--shadow-card`) and, with `interactive`, one hover elevation
 * (`--shadow-card-hover`).
 */
function Card({
  className,
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-4 overflow-hidden rounded-card border border-border bg-card py-4 text-body text-card-foreground shadow-card",
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-card border-t border-border bg-muted/50 p-4 group-data-[size=sm]/card:p-3",
        className
      )}
      {...props}
    />
