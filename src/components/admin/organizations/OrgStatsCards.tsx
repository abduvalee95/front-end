...
interface StatConfig {
  ...
  getSub: (ctx: { total: number; active: number; inactive: number; users: number; rate: number }) => string;
...
const STAT_CONFIGS: StatConfig[] = [
  {
    ...
    getSub: () => 'Registered on the platform',
    icon: Building2,
  ...
  },
  {
    ...
    getSub: ({ rate }) => `${rate}% activation rate`,
    icon: CheckCircle2,
  ...
  },
  {
    ...
    getSub: ({ total, rate }) => total > 0 ? `${100 - rate}% suspended` : 'None suspended',
    icon: XCircle,
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {STAT_CONFIGS.map((cfg) => {
        ...
        const sub = cfg.getSub(ctx);
        return (
          <Card key={cfg.label} className="overflow-hidden border-0 shadow-sm">
