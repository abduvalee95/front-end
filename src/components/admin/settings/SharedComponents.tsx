...
export function SectionHeader({ icon, title, desc }: SectionHeaderProps) {
  return (
    <div className="flex items-center gap-3 pb-1">
      ...
      <div>
        <h2 className="text-h3 text-foreground leading-tight">{title}</h2>
    ...
    <div className="flex items-center justify-between px-4 py-3.5 rounded-xl border border-border/60 bg-muted/30 hover:bg-muted/60 transition-colors">
      <div className="space-y-0.5 min-w-0 pr-4">
        <div className="flex items-center gap-2">
          ...
          <Label className={`text-h4 cursor-pointer ${danger ? 'text-danger-emphasis' : 'text-foreground'}`}>{title}</Label>
          {danger && checked && (
...
export function SaveButton({ isSaving, onClick, label }: SaveButtonProps) {
  ...
  return (
    <Button
      ...
      onClick={onClick}
      disabled={isSaving}
      className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl px-6 shadow-md gap-2 text-body shrink-0"
    >
  ...
  return (
    <Card className="rounded-2xl border-border shadow-sm h-fit">
      <CardHeader className="pb-3">
        <CardTitle className="text-h4">{t('settings.security_score')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className={`text-display text-center tabular-nums ${color}`}>{pct}%</div>
        <div className="h-2 rounded-full bg-muted overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-700 ${barColor}`} style={{ width: `${pct}%` }} />
        </div>
        <div className="space-y-2 pt-1">
          {checks.map((c) => (
            <div key={c.label} className="flex items-center gap-2 text-caption">
              {c.ok
