            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueData} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ borderRadius: '12px', border: '1px solid hsl(var(--border))', background: 'hsl(var(--popover))', color: 'hsl(var(--popover-foreground))', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}
          ...
          <div className="bg-card rounded-2xl p-6 h-full shadow-sm">
            ...
            <div className="space-y-1">
              {summaryLoading ? (
                ...
                summary.upcomingLessons.slice(0, 5).map((lesson) => (
                  <div key={lesson.id} className="group flex gap-3 rounded-xl p-2.5 cursor-pointer hover:bg-accent transition-colors">
                    ...
                    <div className="min-w-0">
                      <p className="text-[12px] font-semibold text-foreground group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate">{lesson.title}</p>
            ...
            <div className="mt-5 pt-5 border-t border-border">
              <div className="rounded-xl bg-gradient-to-br from-muted to-indigo-50/60 dark:to-indigo-500/5 p-4">
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground mb-3">{t('overall_attendance')}</p>
                {summaryLoading ? (
                  ...
                  <div className="flex items-baseline gap-1.5 mb-3">
                    ...
                    <span className="text-[10px] font-medium text-muted-foreground ml-1">{summary?.attendancePresent || 0} {t('present')}</span>
                  </div>
                )}
...
function StatCard({ title, value, unit, subtitle, icon: Icon, trend, trendUp, color }: StatCardProps) {
  return (
    <div
      className="group relative bg-card rounded-2xl p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 overflow-hidden"
    >
      {/* Top accent line */}
      <div
